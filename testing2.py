...
MOD_FREQ_HIGH = 17  # High modulation frequency in Hz
MOD_FREQ_LOW = 10  # Low modulation frequency in Hz
NUM_BLOCKS = 6
TRIALS_PER_BLOCK = 40
BREAK_TIME = 5  # Break duration in seconds
VISUAL_FREQ_HIGH = 20  # High visual flicker frequency in Hz
VISUAL_FREQ_LOW = 12  # Low visual flicker frequency in Hz
...
# Precompute tones
tone_high = create_modulated_tone2(MOD_FREQ_HIGH)
...
class RestingState:
    ...
    def start(self):
        ...
        if not self.eyes_open:  # Only play sound if eyes were closed
            ...
            FREQUENCY = 1000  # Frequency of the beep sound in Hz (A4 note)
...
run_visual_training(win, VISUAL_FREQ_HIGH, VISUAL_FREQ_LOW, 1,
                    "Focus on the left-side high-frequency stimuli.")
run_visual_training(win, VISUAL_FREQ_LOW, VISUAL_FREQ_HIGH, 1,
