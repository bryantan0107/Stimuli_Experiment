...
def show_pre_experiment_check(win):
    ...
    checklist_text = visual.TextStim(
        win,
        text=("Before starting the experiment, please confirm the following with the experimenter:\n\n"
...
class VisualStimulus(Stimulus):
    def __init__(self, win, trigger_sender, left_freq, right_freq, sf=ExperimentConfig.SF, size=300):
        ...
        self.left_grating = visual.GratingStim(
            win, tex="sin", mask="gauss", sf=sf, size=size, pos=(-200, 0))
...
class Experiment:
    ...
    def run_practice_session(self, focus_type, trials):
        ...
        practice_instruction_text = {
            "Visual first, then Auditory": "This is a practice session where you should focus on the visual stimuli first.\n\n"
                                           "Press any key to start the practice session.",
            "Auditory first, then Visual": "This is a practice session where you should focus on the auditory stimuli first.\n\n"
                                           "Press any key to start the practice session."
        }
        ...
        while True:
            feedback_text = "Did you feel comfortable completing the task?\n\n" \
    ...
    def run_trial(self, trial_index, trial, block):
        """
